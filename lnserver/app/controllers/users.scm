...
(define (prep-user-rows a)
  (fold (lambda (x prev)
		...
		(group-name (result-ref x "usergroup"))
            ...
            (cons (string-append "<tr><th> <input type=\"radio\" id=\"" id  "\" name=\"id\" value=\"" id   "\"></th><th> " id "</th><th> " group-name "</th><th> " lnuser-name "</th></tr>")
		  prev)))
        '() a))
...
(users-define getall
    ...
    (let* ((help-topic "users")
	   (sql "select person.id, person.usergroup, lnuser from person ORDER BY person.id DESC"  )
...
(get "/users/addaction" #:conn #t
  ...
  (let*((salt (get-random-from-dev #:length 8 #:uppercase #f))
	...
	(sql (string-append "insert into person (lnuser, passwd, salt, usergroup ) VALUES ('" name "', '" pwd "', '" salt "', '" usergroup "')"))
	(dummy (:conn rc sql)))
